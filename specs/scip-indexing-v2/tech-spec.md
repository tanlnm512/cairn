...
### D-013: execute waves after wave 1 gate on landing evidence, not ticks
- **Context**: ticks are forbidden until the closing audit, so the mechanical per-task frontier (deps ticked/struck) holds every chained task `blocked (unticked)` once wave 1's 10 tasks were implemented — the workflow runner correctly parked at its no-change stop.
- **Decision**: per the chained-task rule, the orchestrator verifies each landed upstream (digest + scoped re-run of its proof anchors: 68-test anchor battery, scaling gate 5-passed, direct CLI/docs/extra checks, 2026-09-20) and spawns dependents on that evidence; the mechanical frontier is the floor, not a veto.
- **Consequences**: waves 2+ are orchestrator-spawned; no task is ticked early; the closing audit still runs exactly once over the whole plan.

D-008 re-pin (T004, 2026-09-20, measured on feat/scip-indexing-v2 @ ac20f609 + T001/T002 graph rewrites in tree): gate body run twice at the 1000-file medium point — wall 6.021 s / 6.057 s (budget 60 s), peak traced 228.25 MB / 228.21 MB (budget 512 MB), 700,000 post-synthesis structural edges (5× over 140,000 base; 31,001 symbols). Variance ≤0.6% wall, 0.02% peak. Gate test green: tests/test_scaling_gate.py::test_closure_budget_gate_at_1000_files 1 passed in 41.70 s.

### D-014: three TC commands adjudicated at the closing audit (behavior proven elsewhere)
- **Context**: proofs rerun leaves TC-009/TC-011/TC-021 red verbatim on this machine; in each case the code behavior is green elsewhere and only the shell command is environment- or template-broken.
- **Decision**: TC-009's `[ "$(wc -l < f)" = "1" ]` fails on BSD wc (leading-space padding; passes on Linux CI) — generate-exactly-once is pinned by test_builder_scip_overlay.py::test_absent_index_generated_exactly_once_then_imported and TC-010's standing guard (PASS). TC-011's guard `[ -z "$(command -v scip-python)" ]` trips over a real /opt/homebrew/bin/scip-python on this dev machine — the missing-binary behavior is TC-012 (PASS) plus the pytest degrade arms. TC-021's command is a `<placeholder>` template — the pinned eval is tests/scip_eval_harness.py (report schema scip-zero-false-exact/1, false_exact=0 asserted) and tests/test_scip_integration.py, both green.
- **Consequences**: qa follow-up owed: BSD-portable comparisons in test.md commands, a PATH-pinned no-binary guard, and a concrete TC-021 invocation; none block this plan — the underlying FRs are proven by the cited runs.

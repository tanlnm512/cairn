...
- Task entry (verbatim):
   - [ ] T009 (in-progress) Wire enrichment into the pipeline and emitter; wave-C integration of rank × enrichment (after T006 + T008) — `src/cairn/pack.py` (FR-003, FR-004)
- spec_dir: specs/context-pack
...
- Acceptance commands (always for code tasks — run them while implementing; the closing audit re-runs them):
   TC-004: `cairn pack --task "retry backoff policy with callers and past mistakes" --budget 6000`
   TC-005: `cairn pack --task "compress the repo map cluster output" --budget 2500`
   TC-006: `cairn pack --task "the graph closure builder routine" --budget 2000`

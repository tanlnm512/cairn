# Spawn payload — implementer · node execute · wave 11
...
- Task entry (verbatim):
   - [ ] T001 (fix 1/5) [P] Create the pack pipeline module: item model, cost hook, seed stage — `src/cairn/pack.py` (FR-001, FR-005)
- spec_dir: specs/context-pack
...
- Acceptance commands (always for code tasks — run them while implementing; the closing audit re-runs them):
   TC-001: `rm -rf /tmp/qacp && F=/tmp/qacp && mkdir -p $F/repo/.git $F/knowledge/compass && printf '%s
   TC-002: `rm -rf /tmp/qacp && F=/tmp/qacp && mkdir -p $F/repo/.git $F/knowledge/compass && printf '%s
   TC-003: `rm -rf /tmp/qacp && F=/tmp/qacp && mkdir -p $F/repo/.git $F/knowledge/compass && printf '%s
   TC-009: `rm -rf /tmp/qacp && F=/tmp/qacp && mkdir -p $F/repo/.git $F/knowledge/compass && printf '%s
   TC-011: `rm -rf /tmp/qacp && F=/tmp/qacp && mkdir -p $F/repo/.git $F/knowledge/compass && printf '%s
   TC-012: `/usr/bin/time -p env CAIRN_WORKSPACE=/tmp/qacp/repo /Users/tanle/Projects/cairn/.venv/bin/cairn pack --task "retry backoff" --budget 3000 --db /tmp/qacp/graph.db --knowledge /tmp/qacp/knowledge >/dev/null 2>&1`
   TC-013: `rm -rf /tmp/qacp && F=/tmp/qacp && mkdir -p $F/repo/.git $F/knowledge/compass && printf '%s

# Spawn payload — implementer · node execute · wave 10
...
- Task entry (verbatim):
   - [ ] T011 (in-progress) Run the assembled bench arm and record the fit-rate numbers (after T009 + T010) — measurement run over `src/cairn/bench/agent_suite.py` as registered near `src/cairn/cli/bench.py:190`; no product file changes (FR-001, FR-002, FR-004)
- spec_dir: specs/context-pack
...
- Acceptance commands (always for code tasks — run them while implementing; the closing audit re-runs them):
   TC-001: `cairn pack --task "how does the retry backoff policy handle transient failures" --budget 4000`
   TC-002: `cairn pack --task "token budget fitting for context packs" --budget 3000`
   TC-003: `cairn pack --task "trace what happens when a wiki page promotion is rejected by the critic" --budget 5000`
   TC-007: `cairn pack --task "memory promotion scoring" --budget 900`
   TC-008: `cairn pack --task "retry backoff policy with callers and past mistakes" --budget 50`
   TC-009: `cairn pack --task "" --budget 1000`
   TC-010: `cairn pack --task "retry backoff policy" --budget 0`
   TC-012: `time cairn pack --task "request routing and validation" --budget 3000`
   TC-013: `cairn pack --task "configuration loading flow" --budget 2500 > /tmp/qa-pack-a.md & cairn pack --task "configuration loading flow" --budget 2500 > /tmp/qa-pack-b.md & wait; diff /tmp/qa-pack-a.md /tmp/qa-pack-b.md`

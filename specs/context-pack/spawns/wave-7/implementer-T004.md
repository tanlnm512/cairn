...
- Task entry (verbatim):
   - [ ] T004 (in-progress) Add the `cairn pack` CLI command and register it (after T003) — `src/cairn/cli/pack.py` new + one import line in `src/cairn/cli/__init__.py` (FR-001, FR-005)
- spec_dir: specs/context-pack
...
- Acceptance commands (always for code tasks — run them while implementing; the closing audit re-runs them):
   ...
   TC-011: `cairn pack --task "retry backoff policy" --budget 1500 > /tmp/qa-pack-run1.md && HTTPS_PROXY=http://127.0.0.1:9 HTTP_PROXY=http://127.0.0.1:9 cairn pack --task "retry backoff policy" --budget 1500 > /tmp/qa-pack-run2.md && diff /tmp/qa-pack-run1.md /tmp/qa-pack-run2.md`

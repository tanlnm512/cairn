...
- [ ] T001 [P] Create the pack pipeline module: item model, cost hook, seed stage — `src/cairn/pack.py` (FR-001, FR-005)
...
- [ ] T002 Add capped one-hop precise expansion to the pipeline (after T001) — `src/cairn/pack.py` (FR-001)
...
- [ ] T003 Add the markdown emitter and header token accounting (after T002) — `src/cairn/pack.py` (FR-001, FR-005)
...
- [ ] T004 Add the `cairn pack` CLI command and register it (after T003) — `src/cairn/cli/pack.py` new + one import line in `src/cairn/cli/__init__.py` (FR-001, FR-005)
...
- [ ] T005 [P] Rank the expanded symbol set by structural centrality — `src/cairn/pack.py` (FR-002)
...
- [ ] T006 Add the budget fitter with centrality-ordered graceful degradation (after T005) — `src/cairn/pack.py` (FR-004)
...
- [ ] T007 [P] Create the enrichment module: source trimmer + per-symbol blast-radius line — `src/cairn/pack_enrich.py` (FR-003)
...
- [ ] T008 Add compass-excerpt and top-3 memory picks to the enrichment module (after T007) — `src/cairn/pack_enrich.py` (FR-003)
...
- [ ] T009 Wire enrichment into the pipeline and emitter; wave-C integration of rank × enrichment (after T006 + T008) — `src/cairn/pack.py` (FR-003, FR-004)
...
- [ ] T010 [P] Add the bench fit-rate task pair to the agent suite — `src/cairn/bench/agent_suite.py` (FR-001, FR-004)
...
- [ ] T011 Run the assembled bench arm and record the fit-rate numbers (after T009 + T010) — measurement run over `src/cairn/bench/agent_suite.py` as registered near `src/cairn/cli/bench.py:190`; no product file changes (FR-001, FR-002, FR-004)

...
- Task entry (verbatim):
   - [ ] T009 (in-progress) Author `.github/workflows/review.yml`: on pull_request events install the repo's cairn CLI in CI (pin the install method — tech-spec pitfall), run `cairn review --pre-submit` without `--gate` (D-004), and post the printed findings as a PR comment; a findings-free run posts no comment (the TC-013 shape)
- spec_dir: specs/pr-review-loop

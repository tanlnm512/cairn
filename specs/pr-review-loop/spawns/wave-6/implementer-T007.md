...
- Task entry (verbatim):
   - [ ] T007 (in-progress) Implement the `--pre-submit` mode in `src/cairn/cli/review.py` plus the engine: reuse T002's diff-to-seed path (`compute_blast` result `seeds`), query `search_memory` (`src/cairn/memory/promotion.py:242`) for type `mistake` and `pattern` keyed to the seed symbols, and print one warning per match with its recorded guidance, formatted through T003's emitter; exit 0 with zero or more warnings (D-004)
- spec_dir: specs/pr-review-loop

...
- Task entry (verbatim):
   - [ ] T004 (in-progress) [P] Implement the comment-to-symbol span mapper in `src/cairn/review/`: map a comment's file+line to the enclosing symbol(s) via the stored `line_start`/`line_end` spans (`src/cairn/graph/schema.py:42-43`), degrading to file-level keying when no span encloses the line (spec risk mitigation); unit tests over a fixture index prove both paths
- spec_dir: specs/pr-review-loop

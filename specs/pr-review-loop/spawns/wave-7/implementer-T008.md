...
- Task entry (verbatim):
   - [ ] T008 (in-progress) Implement the `--gate` flag for `--pre-submit` per D-004: exit non-zero only when `--gate` is set and at least one mistake/pattern match exists; without the flag the run stays exit 0 however many warnings print
- spec_dir: specs/pr-review-loop

...
- Task entry (verbatim):
   - [ ] T010 (in-progress) (after T008) Add the Phase 2 surface assertions — new tests/test_taint_surfaces.py covering TC-007 and TC-008 (FR-004)
- spec_dir: specs/taint-tracking
...
- Acceptance commands (always for code tasks — run them while implementing; the closing audit re-runs them):
   TC-007: `cd specs/taint-tracking/fixtures/change-intersect && cairn blast run_query | tee /tmp/tc007.out >/dev/null && grep -qi taint /tmp/tc007.out && ! cairn blast format_label | grep -qi taint`
   TC-008: human observation — in an MCP client, query explore for the SQL-executing function in the change-intersect fixture and verify the response contains a taint warning naming the path; repeat for the off-path helper and verify no warning appears

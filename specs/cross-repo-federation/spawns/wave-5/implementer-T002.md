...
- spec_dir: specs/cross-repo-federation
- repo: .
...
- Task entry (verbatim):
   - [ ] T002 (in-progress) Add per-store state classification and the named dropped-store report to the federation core in `src/cairn/graph/federation.py` (FR-004) (after T001)
- spec_dir: specs/cross-repo-federation
...
- Acceptance commands (always for code tasks — run them while implementing; the closing audit re-runs them):
   TC-007: `python3 -m unittest tests.test_federation.UnavailableStoreTests.test_missing_store_is_named`
   TC-008: `python3 -m unittest tests.test_federation.UnavailableStoreTests.test_unindexed_store_is_named`
   TC-009: `python3 -m unittest tests.test_federation.UnavailableStoreTests.test_locked_store_is_named`
   TC-010: `python3 -m unittest tests.test_federation.UnavailableStoreTests.test_empty_registry_reports_clearly`

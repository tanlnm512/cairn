...
- Task entry (verbatim):
   - [ ] T008 (in-progress) Run the regression gate — full suite plus survey verify commands green, single-store behavior unchanged (FR-005)
- spec_dir: specs/cross-repo-federation
...
- Acceptance commands (always for code tasks — run them while implementing; the closing audit re-runs them):
   TC-011: `/Users/tanle/Projects/cairn/.venv/bin/python -m unittest tests.test_federation.SingleStoreBaselineTests.test_default_commands_stay_single_store`

...
- Task entry (verbatim):
   - [ ] T006 (in-progress) Add the opt-in `--shared-embed` backend-sharing mode to the federation core in `src/cairn/graph/federation.py` (FR-002) (after T002)
- spec_dir: specs/cross-repo-federation
...
- Acceptance commands (always for code tasks — run them while implementing; the closing audit re-runs them):
   TC-004: `python3 -m unittest tests.test_federation.EmbeddingBackendTests.test_shared_backend_option_returns_attributed_results`
   TC-005: human observation — index two stores with different

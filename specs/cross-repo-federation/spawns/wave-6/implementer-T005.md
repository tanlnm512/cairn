...
- Task entry (verbatim):
   - [ ] T005 (in-progress) Add the default-off `--all-repos` flag to `cairn ask` in `src/cairn/cli/ask_context.py` (FR-003) (after T002)
- spec_dir: specs/cross-repo-federation
...
- Acceptance commands (always for code tasks — run them while implementing; the closing audit re-runs them):
   TC-006: `python3 -m unittest tests.test_federation.FederatedAskTests.test_ask_all_repos_attributes_context_per_repo`
   TC-012: `python3 -m unittest tests.test_federation.FederatedAskTests.test_blank_input_rejected_before_querying`

...
- Task entry (verbatim):
   - [ ] T003 (in-progress) [P] Expose the read-only MCP tool `federated_search` — new `src/cairn/mcp_server/tools_federation.py` plus registration (FR-001) (after T002)
- spec_dir: specs/cross-repo-federation
...
- Acceptance commands (always for code tasks — run them while implementing; the closing audit re-runs them):
   TC-001: `python3 -m unittest tests.test_federation.FederatedSearchTests.test_merged_hits_carry_repo_attribution`
   TC-002: `python3 -m unittest tests.test_federation.FederatedSearchTests.test_lexical_only_store_still_contributes`
   TC-003: `python3 -m unittest tests.test_federation.FederatedSearchTests.test_tool_and_cli_surfaces_agree`

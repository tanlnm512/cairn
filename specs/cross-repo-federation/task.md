...
<!-- Checkpoint: a federated query over fixture stores (2+ indexed, plus 1
     ...
     still pass. -->
- [ ] T001 Build the federation core query path — fan-out, RRF fusion, per-repo attribution — in a new `src/cairn/graph/federation.py` (FR-001)
...
- [ ] T002 Add per-store state classification and the named dropped-store report to the federation core in `src/cairn/graph/federation.py` (FR-004) (after T001)
...
- [ ] T003 [P] Expose the read-only MCP tool `federated_search` — new `src/cairn/mcp_server/tools_federation.py` plus registration (FR-001) (after T002)
...
- [ ] T004 [P] Add the CLI command `cairn federated-search` to `src/cairn/cli/query.py` (FR-001) (after T002)
...
- [ ] T005 Add the default-off `--all-repos` flag to `cairn ask` in `src/cairn/cli/ask_context.py` (FR-003) (after T002)
...
- [ ] T006 Add the opt-in `--shared-embed` backend-sharing mode to the federation core in `src/cairn/graph/federation.py` (FR-002) (after T002)
...
- [ ] T007 Wire `--shared-embed` through both federation surfaces (FR-002) (after T006)
...
- [ ] T008 Run the regression gate — full suite plus survey verify commands green, single-store behavior unchanged (FR-005)

...
- [ ] T001 Add `agent_symbols` table and index to `_apply_schema` in src/cairn/graph/schema.py (FR-005, D-002)
...
- [ ] T002 [P] Add `validate_agent_id` helper to src/cairn/memory/store.py (FR-006, D-001)
...
- [ ] T003 (after T001, T002) Add `share_memory` beside `create_memory`/`store_memory` in src/cairn/memory/store.py (FR-005, D-004)
  ...
  - anchor: `pytest tests/test_memory_store_protocol.py tests/test_memory_store_fixes.py`
- [ ] T004 (after T003) Add two-concurrent-simulated-agents test module tests/test_memory_share_concurrency.py (FR-005, FR-004)
...
- [ ] T005 (after T002, T003) Add `share` verb to the `memory` CLI group in src/cairn/cli/memory.py (FR-001)
...
- [ ] T006 (after T005) Add `--agent` read-through to `cairn memory search` in src/cairn/cli/memory.py (FR-003, D-003)
  ...
  - anchor: `pytest tests/test_cli_smoke.py tests/test_memory_mcp_trim.py`
- [ ] T007 [P] (after T003) Add optional trailing `agent` parameter to `recall_memory` in src/cairn/mcp_server/tools_memory.py (FR-003, FR-004, D-003)
...
- [ ] T008 (after T006, T007) Add share-to-recall integration tests in tests/test_memory_share_recall.py (FR-001, FR-003)
...
- [ ] T009 (after T003) Add `check_overlap` beside `create_memory`/`store_memory` in src/cairn/memory/store.py (FR-002, D-002)
  ...
  - anchor: `pytest tests/test_memory_store_protocol.py tests/test_memory_store_fixes.py`
- [ ] T010 (after T005, T009) Add `check` verb to the `memory` CLI group in src/cairn/cli/memory.py (FR-002)
  ...
  - anchor: `pytest tests/test_cli_smoke.py tests/test_memory_mcp_trim.py`
- [ ] T011 (after T010) Add end-to-end overlap tests in tests/test_memory_overlap_check.py (FR-002, FR-004)

...
- Task entry (verbatim):
   - [ ] T008 (in-progress) (after T006, T007) Add share-to-recall integration tests in tests/test_memory_share_recall.py (FR-001, FR-003)
- spec_dir: specs/shared-memory-bus
...
- Acceptance commands (always for code tasks — run them while implementing; the closing audit re-runs them):
   TC-001: `cairn memory share --agent agentA --symbols auth,session && cairn memory search auth`
   TC-002: `cairn memory share --agent agentA --symbols auth && cairn memory share --agent agentA --symbols auth && cairn memory search auth`
   TC-003: human observation — run the share with an empty `--symbols` value and inspect the outcome against the Then above.
   TC-004: `cairn memory share --agent agentA --symbols $(python3 -c "print(','.join('sym%02d' % i for i in range(50)))") && cairn memory search sym37`
   TC-009: `cairn memory share --agent agentA --symbols session && cairn memory search session`

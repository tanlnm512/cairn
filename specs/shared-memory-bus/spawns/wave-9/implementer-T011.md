...
- Task entry (verbatim):
   - [ ] T011 (in-progress) (after T010) Add end-to-end overlap tests in tests/test_memory_overlap_check.py (FR-002, FR-004)
- spec_dir: specs/shared-memory-bus
...
- Acceptance commands (always for code tasks — run them while implementing; the closing audit re-runs them):
   TC-005: `cairn memory share --agent agentA --symbols auth,session && cairn memory check --agent agentB --symbols auth,users`
   TC-006: `cairn memory share --agent agentA --symbols auth && cairn memory check --agent agentB --symbols parser,wiki`
   TC-007: `cairn memory share --agent agentB --symbols auth && cairn memory check --agent agentB --symbols auth`
   TC-008: human observation — perform a short editing session as agentA on a fixture symbol, then run the overlap check as agentB for that symbol and confirm the warning names agentA.

# Wave 1 — Stage 1 (survey ∥ research)

- surveyor · owns survey.md · mission: code-state ground truth for FR-001..FR-007 (query path, embedding chunk recipe, fusion/threshold knobs, reranker pair construction, eval harness splits, latency baselines to protect)
- researcher · owns research.md · mission: RQ1–RQ5 external evidence (query-doc mismatch, hybrid fusion tuning, chunk composition, reranker pair formats, small-GT methodology)

## Shared protocol (verbatim — every payload carries it by reference to this file)
...
- planner · owns plan.md · mission: milestones + parallelization map for FR-001..007 from spec.md + survey.md
- tech · owns tech-spec.md · mission: architecture, D-### decisions over research.md's 6 option sets, impact analysis, diagram
- qa · owns test.md · mission: TC-### business cases traced to FR/AC (blind to tech-spec.md)

# Wave 3 — Stage 3 (task-breaker, single writer)
- task-breaker · owns task.md · mission: phased task list from plan.md milestones + tech-spec code guide (D-001..D-008), statuses from survey.md only, [P] by default

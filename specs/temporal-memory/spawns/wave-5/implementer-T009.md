...
- Acceptance commands (always for code tasks — run them while implementing; the closing audit re-runs them):
   TC-005: observation — after the rename and validation run, the memory list still contains the record marked expired at the build time, a successor reference is shown, an unfiltered search for it returns nothing, and an as-of search dated before the build returns it
   TC-006: observation — the record is marked expired at the build time and carries no successor reference

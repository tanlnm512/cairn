...
- Acceptance commands (always for code tasks — run them while implementing; the closing audit re-runs them):
   TC-003: `bash -c 'T=$(mktemp -d) && mkdir -p "$T/k" && cairn memory search "nothing here" --as-of 2000-01-01 --db "$T/g.db" --knowledge "$T/k" >/dev/null 2>&1'`
   TC-009: (observation only — MANUAL)
   TC-010: `cairn bench --suite perf --baseline --n-files 20 --complexity low`

...
- Task entry (verbatim):
   - [ ] T008 (in-progress) Record after-benchmark numbers + delta in tech-spec.md's FR-005 table — same D-006 protocol on the feature branch; fill After and Delta columns; regression bar: p95 delta beyond 2× run-to-run stddev of the before-run. (after T006, T007 — numbers are only comparable after the query-path changes land). (FR-005)
- spec_dir: specs/temporal-memory

...
### D-011: T002 spike verdict splits D-006 — claude global scope embeds via `-e`, droid CLI keeps the skip
- **Context**: D-006 assumed both external registration CLIs (`claude mcp add --scope user`, `droid mcp add`) could not be trusted with env (survey Unknown #3, no evidence). The T002 spike (2026-08-28, this host) produced evidence: `claude mcp add` documents and verifiably persists `-e/--env KEY=value` into the user-scope `mcpServers` entry of `~/.claude.json` (throwaway server `cairn-spike-envtest`, read-back verbatim, then removed; Claude Code v2.1.250); `droid` is not installed on the host (exit 127), so its CLI contract stays unverified.
- **Decision**: T007 implements a split — the claude global-scope registration appends `-e CAIRN_HOME=<expanded>` to its `claude mcp add` argv when the home is non-default; the droid CLI path keeps D-006 unchanged (no embedding attempt, WARN note, no spawn-verify). The droid *file fallback* (droid.py:67) is a cairn writer and follows the normal FR-001 env block like every other file-based generator.
- **Consequences**: claude global-scope users on custom homes get a verified env-bearing registration instead of a WARN; if a future droid CLI gains env support, the spike re-runs and D-011/D-006 are superseded by a fresh decision. Install-time verification (Phase 5) still spawns only file-written registrations; CLI-registered paths stay skipped with a note per D-006/D-005.

### D-012: _already_installed must compare env in the zcode and opencode/kilo branches (T005 finding)
- **Context**: T005 found `_already_installed` (merge.py) compares `env` only in the flat `mcpServers` branch (merge.py:256-258); the zcode branch and the opencode/kilo command-array branch compare command only. With FR-001 now embedding `env.CAIRN_HOME`, a reinstall after the user MOVES the store (changed CAIRN_HOME) keeps the stale env in those three shapes' configs — the exact silent-wrong-store class this spec exists to kill.
- **Decision**: fold the fix into the plan as T023 (Phase 1 scope, FR-001): extend the env comparison to the zcode and opencode/kilo branches, with tests in test_atomic_config_writes.py pinning changed-env ⇒ reinstall rewrites (and default-home ⇒ env key removed on reinstall).
- **Consequences**: one small task on merge.py + one test file; idempotence for unchanged env stays byte-stable. If skipped, Phase 5's install-verify would catch stale env only at install time, not on re-runs — unacceptable for the spec's own scenario (moving the store).

### D-013: verify_registration pins the intended env over the merged written env
- **Context**: T019 found the install-verify probe must pin the intended CAIRN_HOME/CAIRN_WORKSPACE (derived from the installer's expected store) over the merged written env — otherwise the test-harness environment (conftest re-points bindings while os.environ diverges) makes a healthy install unprovable, and PATH-shadow/env-dropping wrappers (TC-011) would not be caught.
- **Decision**: install-time verification proves "the registration's exact binary resolves the intended store" (catches wrong binaries and env-dropping wrappers); it deliberately does NOT prove "the registration's env block is complete" — merely-missing env on a stale registration remains the doctor's WARN sub-audit (D-007's mixed severity).
- **Consequences**: the two verification layers are complementary by design: install-verify FAILs on provable wrong-store, doctor WARNs on missing-env and FAILs on a registration that resolves a different existing store or an unreachable SSE endpoint. If a future need arises to prove env completeness at install time, the pin must become conditional, not removed.

...
- [x] T007 [P] Add the modern-syntax fixture set — new `tests/fixtures/kotlin/modern/` holding one .kt fixture per FR-004 construct: KEEP-0438 destructuring, `when` guard conditions, multi-dollar string interpolation, trailing commas (FR-004)
...
- [x] T008 Add the ERROR/MISSING scan test — new test module `tests/test_kotlin_modern_syntax.py` that parses each fixture and asserts `not tree.root_node.has_error` plus zero MISSING nodes, enumerating the fixture directory so every fixture is covered mechanically (after T002, T007) (FR-004; D-005)
...
## Done proofs (closing audit 2026-08-25)
<!-- Per-task proof commands, all run this session with green output. -->
- T001 — done 2026-08-25: 8 vendored files byte-identical to pin 1852ea1 (`cmp`); NOTICE block records origin/pin/license (D-010)
- T002 — done 2026-08-25: `Parser(Language(cairn._tree_sitter_kotlin.language())).parse(...)` clean on `class Foo` + multi-dollar fixture `has_error False` (old grammar: True); baselines exact (26 core / 13 kotlin)
- T003 — done 2026-08-25: `_field_types` rows row-for-row identical old vs new (Identifiable/Status/BaseEntity/User probe)
- T004 — done 2026-08-25: `User extends BaseEntity` + `implements Identifiable` + `open` modifier restored under fwcd shapes (pre-port: zero inheritance edges)
- T005 — done 2026-08-25: Import records identical across plain/dotted/aliased/star shapes under fwcd `identifier` nodes
- T006 — done 2026-08-25 (fix 1/5): flip live — 30 passed (hybrid+golden+invoke+self-demo), 18 passed `-k kotlin`, 24 core-smoke, golden byte-clean, 26 `-m core`
- T007 — done 2026-08-25: 4 fixtures parse zero-ERROR under fwcd (standalone-compile check) and ERROR under old 1.1.0 — the delta is real
- T008 — done 2026-08-25: scan test 7 passed (6 fixtures + guard); sabotage run FAILED as designed then reverted — detection proven
- T009 — done 2026-08-25: `uv build` → `cairn_intel-0.13.0-cp310-abi3-macosx_11_0_arm64.whl` + sdist carrying vendored sources; workflow yaml ok; publish/release jobs untouched
- T010 — done 2026-08-25: `tree-sitter-kotlin` absent from pyproject/uv.lock/NOTICE; `import tree_sitter_kotlin` → ModuleNotFoundError; 18 kotlin tests green post-removal
- T011 — done 2026-08-25: 30 passed hybrid gate + full suite 2135 passed on the new grammar (re-run at audit: 2137 passed, 1 skipped)
- T012 — done 2026-08-25: `[Unreleased]` Added+Changed entries present (TC-019 auto PASS)

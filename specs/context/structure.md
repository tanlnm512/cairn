...
enumeration re-counted; all other cited lines re-verified unchanged)
...
## Package facts
- Distribution `cairn-intel` v0.20.0 (pyproject.toml:7; was v0.18.0 at last refresh), src-layout (`[tool.setuptools.packages.find] where = ["src"]`,
...
1. `cairn` CLI → `src/cairn/cli/__init__.py:15` (`from .main import main`) → the click
   ...
   subcommands onto it (re-counted @ d18768c: 24 single-file command modules plus the
   `system/` package — enumerate with `grep -l "@main" src/cairn/cli/*.py`; new in this
   delta: `skill.py`, the `cairn skill generate` group).

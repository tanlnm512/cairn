...
- [x] T004 (after T003) Expose k-fold mode in `eval_cmd` (`src/cairn/cli/system.py`) — configurable fold count, fold-level rows in the emitted sweep doc; verify: `uv run cairn eval --sweep <spec> --queries benchmarks/datasource/t2/ground_truth --out /tmp/kfold-sweep.json` shows fold-level rows; targeted pytest for fold loop + guard green (FR-001)
  done 2026-08-17 — real CLI run: `cairn eval --db … --sweep … --kfold --queries t2/ground_truth --out …` → 5 folds, 10 rows, aggregate with spread (pooled bootstrap n=82); tests 44 passed (folds 2/4 rejected naming the floor); single-split path byte-identical
...
- [x] T010 (after T009) Verify and seal DS-v2 — empirically verify every expectation against a fresh graph build (zero aspirational entries) and commit the provenance artifact beside the dataset (per-kind/level counts, pass rate 100%, the build facts verified against); pin tree_hash and the manifest dataset_version; DS-v1 artifacts byte-identical throughout (TC-006, TC-007) (FR-002)
  done 2026-08-17 — VERIFICATION.{md,json} (cairn-ds2-verification/1): 558/558 tier-1-exact, pass rate 1.0000, zero aspirational; manifest pinned (dataset_version DS-v2; tree_hash per the existing cairn.bench.datasource.tree_hash mechanism); durable verify_dataset.py; 12-query spot-check agrees; t2/ + ablation v1 byte-identical

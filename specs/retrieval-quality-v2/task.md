...
- [ ] T001 (in-progress) Add `kfold_partitions` to `src/cairn/eval.py` — sorted-dedupe-seeded-shuffle then k contiguous rotation slices (the `split_queries` determinism recipe over `DEFAULT_SPLIT_SEED`); every id held out exactly once; refuse k < 5 with an error naming the floor; tests: regenerate-and-diff empty, union of held-out folds == full id set, no overlap (FR-001)

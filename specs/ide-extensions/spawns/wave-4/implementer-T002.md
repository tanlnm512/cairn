...
- Task entry (verbatim):
   - [ ] T002 (in-progress) [P] Pin the frozen editor JSON contract with contract tests (FR-003, FR-006)
- spec_dir: specs/ide-extensions
...
- Acceptance commands (always for code tasks — run them while implementing; the closing audit re-runs them):
   TC-007: human observation — on a clean machine the walkthrough
   TC-008: `grep -riE "configure mcp|mcp config|cairn (serve|update|index)" <shipped quick-start doc>` — no matches (run from the extension project root; fails if a manual-setup step creeps into the default path)
   TC-013: human observation — all four walkthroughs pass on the

...
- Task entry (verbatim):
   - [ ] T007 (in-progress) [P] Implement the compass/memory panel for the active file (FR-002)
- spec_dir: specs/ide-extensions
...
- Acceptance commands (always for code tasks — run them while implementing; the closing audit re-runs them):
   TC-005: human observation — the panel appears on open and its
   TC-006: human observation — the panel renders an empty state

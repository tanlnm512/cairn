...
- Task entry (verbatim):
   - [ ] T006 (in-progress) [P] Implement the code-lens provider (FR-001)
- spec_dir: specs/ide-extensions
...
- Acceptance commands (always for code tasks — run them while implementing; the closing audit re-runs them):
   TC-001: human observation — the hover renders both counts and
   TC-002: human observation — code-lens counts appear above the
   TC-003: human observation — the radius matches the depth-limited
   TC-004: human observation — the hover renders a readable
   TC-014: human observation — on the large workspace, hover and

...
- Task entry (verbatim):
   - [ ] T009 (in-progress) (after T008) Add release CI and exercise manual vsix install on a clean machine (FR-005)
- spec_dir: specs/ide-extensions
...
- Acceptance commands (always for code tasks — run them while implementing; the closing audit re-runs them):
   TC-012: `vsce package -o cairn-ext.vsix && code --install-extension cairn-ext.vsix && code --list-extensions | grep -i cairn` (run from the extension project root; all three steps succeed in seconds)

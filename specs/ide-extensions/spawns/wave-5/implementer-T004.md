...
- Task entry (verbatim):
   - [ ] T004 (in-progress) (after T003) Implement graceful degradation in the shell status layer (FR-004)
- spec_dir: specs/ide-extensions
...
- Acceptance commands (always for code tasks — run them while implementing; the closing audit re-runs them):
   TC-009: human observation — the status indicator is the only
   TC-010: human observation — after stopping the server, hover and
   TC-011: human observation — a scripted one-minute

...
- [ ] T001 [P] Write failing tests pinning the stdio env-block shape: embedded when non-default, absent when default (FR-001; TC-001, TC-002, TC-003)
...
- [ ] T002 [P] Spike: probe the external registration CLIs' env contract and record the Unknown #3 verdict (FR-001; TC-017)
...
- [ ] T003 Implement the non-default-home predicate helpers in paths.py (after T001) (FR-001; TC-003)
...
- [ ] T004 Embed the env block in mcp_config_json's stdio branch (after T003) (FR-001; TC-001, TC-002)
...
- [ ] T005 Embed the env block in the four custom-shape stdio generators (after T003) (FR-001; TC-001)
...
- [ ] T006 Add CAIRN_HOME alongside the CAIRN_WORKSPACE pin in the claude-desktop generator (after T003) (FR-001; TC-001)
...
- [ ] T007 Resolve env handling for the CLI-registered global claude/droid registrations (after T002) (FR-001; TC-017)
...
- [ ] T008 [P] Write failing tests pinning the CAIRN_HOME assignment inside generated hook command strings and the git post-commit template (FR-002; TC-004)
...
- [ ] T009 [P] Write the first plist EnvironmentVariables assertions — survey gap: none exist today (FR-003; TC-005, TC-006)
...
- [ ] T010 Implement hook env: prefix in _claude_hook_command and an export line in the git post-commit template (after T008) (FR-002; TC-004)
...
- [ ] T011 Implement plist env: render_plist sets EnvironmentVariables.CAIRN_HOME when non-default (after T009) (FR-003; TC-005, TC-006)
...
- [ ] T012 [P] Write failing tests asserting the missing-store error names the resolved path, the env chain, and the remediation (FR-004; TC-007, TC-008)
...
- [ ] T013 Implement the env-resolution-chain renderer in paths.py and enrich the server boot-guard message (after T012) (FR-004; TC-007)
...
- [ ] T014 Add the get_db parent-directory pre-check raising the enriched error (after T013) (FR-004; TC-008)
...
- [ ] T015 [P] Write failing tests for the machine-readable resolution probe (FR-005; TC-009)
...
- [ ] T016 Implement the `--json` flag on cairn config as the resolution probe (after T015) (FR-005; TC-009)
...
- [ ] T017 [P] Write failing tests for install-time per-client verification (FR-006; TC-010, TC-011)
...
- [ ] T018 Add the defaulted verification fields to InstallResult (after T017) (FR-006; TC-010)
...
- [ ] T019 Implement the spawn-probe verify loop in install() plus per-client PASS/FAIL report rendering (after T018) (FR-006; TC-010, TC-011)
...
- [ ] T020 [P] Write failing tests for the doctor environment check and update the frozen sequence test to 10 names (FR-007; TC-012, TC-013, TC-014, TC-015)
...
- [ ] T021 Implement _check_environment: store-existence, platform/transport, and binary-coherence sub-audits (after T020) (FR-007; TC-013, TC-014, TC-016)
...
- [ ] T022 Implement the registration-consistency sub-audit: spawn-probe stdio registrations, SSE reachability, mixed severity (after T021) (FR-007; TC-012, TC-015, TC-016)

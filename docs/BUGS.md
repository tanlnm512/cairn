...
**Fix:** Documented in the SCIP hybrid plan as a "must fix before the hybrid is worth building." The rewrite (not yet implemented) will: build a `{symbol_descriptor → def_symbol_id}` map from definition occurrences, resolve each reference's `target_id` via that map, compute `source_id` from `enclosing_range`, and set `resolution='exact'` only when `target_id` is actually found.
**Prevention:** Never label data `'exact'` or `'verified'` unless the code path that produced it actually performs the verification. The `resolution` column is a trust signal consumed by precise-by-default queries (`get_callers`, `impact_analysis`); false `'exact'` rows silently pollute blast-radius analysis. A future invariant test should assert: `resolution='exact'` implies `target_id IS NOT NULL`.
**Related:** `src/cairn/parsers/scip_importer.py`, `src/cairn/graph/traversal.py:STRUCTURAL_EDGE_KINDS`, `docs/scip-hybrid-plan.md`.

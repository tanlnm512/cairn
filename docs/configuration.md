...
| `ingest` | object `{"skip": {"add": [...], "disable": [...]}, "classification": {...}}` | Workspace overrides for `cairn knowledge ingest`, layered over the built-in defaults. `classification` maps a lowercased title keyword to a doc_type and is checked before the built-in doc-kind map; `skip.add` extends the built-in skip-list with extra glob patterns; `skip.disable` turns built-in skip categories off (e.g. re-admitting changelogs). Everything defaults to empty — a bare workspace with no `ingest` section runs on built-ins alone, and malformed entries are dropped, never raised. Fed pdf/docx conversion additionally requires the optional `[ingest]` extra (`pymupdf4llm` + `mammoth` + `markdownify`); without it those files are skipped with a logged reason. |

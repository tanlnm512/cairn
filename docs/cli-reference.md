...
| `cairn wiki enrich [<page-id>] [--repo R] [--all]` | queue one `wiki-page-enrich` task per promoted page — either a single `page-id` or `--all` (never both), optionally scoped with `--repo`; the task's facts carry page identity and fresh seeds; the completion reads the promoted body, appends the new sections, and merges the new `## Sources` entries. Requires an already-promoted page, and its critic-passing completion appends the new sections to the promoted body and merges the new `## Sources` entries into the frontmatter. Requires an already-promoted page |

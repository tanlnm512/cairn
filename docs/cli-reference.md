...
| `cairn wiki status` | per-page generation state (queued / in-progress / promoted / failed / dropped) with aggregate counts, joined from the manifest and live task state; each page also carries a staleness verdict — `fresh` when its recorded commit sha equals the repo's current HEAD, `stale` when both are present and differ, `unknown` when either is unavailable — with `fresh=… stale=… unknown=…` in the totals line |
| `cairn wiki retry` | re-queue exactly the failed pages as fresh task chains (cumulative attempt count preserved); promoted pages untouched, and dropped tasks stay dropped — drop is terminal |
...
| `cairn wiki enrich [<page-id>] [--repo R] [--all]` | queue one `wiki-page-enrich` task per promoted page — either a single `page-id` or `--all` (never both), optionally scoped with `--repo`; the task's facts carry the page's current body plus fresh seeds, and its critic-passing completion appends the new sections to the promoted body and merges the new `## Sources` entries into the frontmatter. Requires an already-promoted page |

...
# After the Phase 1.3 split, agent_install is a package (src/codegraph/agent_install/).
...
PKG_DIR = Path("src/codegraph/agent_install")
...
expected = {'codegraph', 'codegraph-prep', 'codegraph-ship', 'codegraph-audit', 'codegraph-refresh'}
...
def test_slash_commands_constant_is_used():
    ...
    """
    literal_pattern = r'["\']codegraph["\']\s*,\s*["\']codegraph-prep["\']\s*,\s*["\']codegraph-ship["\']\s*,\s*["\']codegraph-audit["\']\s*,\s*["\']codegraph-refresh["\']'
    inline_matches = []
...
def test_codegraph_prep_grep_count():
    """Verify grep for 'codegraph-prep' returns only 1 match (the constant definition).
    ...
    for src in sorted(PKG_DIR.rglob("*.py")):
        for line in src.read_text(encoding="utf-8").splitlines():
            if "codegraph-prep" in line:
                matches.append((src, line))
    # Should be 1 - only the constant definition
    assert len(matches) == 1, (
        f"grep for 'codegraph-prep' returned {len(matches)} matches; should be 1 (constant only): {matches}"
    )
...
if __name__ == "__main__":
    ...
    test_codegraph_prep_grep_count()

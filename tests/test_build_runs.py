...
def test_two_builds_yield_two_rows(tmp_path):
    ...
    Uses ``repo_filter`` (the on-disk live-write path) like the full-workspace
    path -- which now ALSO accumulates via ``copy_telemetry_tables`` (see
    ``test_full_rebuild_carries_telemetry_history`` below).
    """
...
def test_full_rebuild_carries_telemetry_history(tmp_path):
    ...
    db_path = str(tmp_path / "carry.db")

    # Seed a prior build's analytics directly into the on-disk DB.
    conn = sqlite3.connect(db_path)
    try:
        _apply_schema(conn)
        conn.execute(
            "INSERT INTO build_runs (kind, started_at, duration_s, session_id) "
            "VALUES ('sync', '2026-08-01T00:00:00Z', 1.5, 'seed')"
        )
        conn.execute(
            "INSERT INTO events (ts, name, session_id, attrs) "
            "VALUES (1754000000.0, 'lock_contention', 'seed', "
            "'{\"site\":\"schema.get_db\"}')"
        )
        conn.execute(
            "INSERT INTO tool_metrics (tool_name, session_id, invoked_at, "
            "duration_ms, status) VALUES ('explore', 'seed', 1754000000.0, 3.0, 'ok')"
        )
        conn.commit()
    finally:
        conn.close()
    ...
    assert summary["files"] > 0
    rows = _build_runs_rows(db_path)
    # ...and the history SURVIVED: the seeded sync row plus this build.
    kinds = sorted(r["kind"] for r in rows)
    assert kinds == ["build", "sync"], f"prior build_runs lost: {kinds}"
    events = sqlite3.connect(db_path).execute(
        "SELECT name FROM events"
    ).fetchall()
    assert [e[0] for e in events] == ["lock_contention"], "prior events lost"
    tools = sqlite3.connect(db_path).execute(
        "SELECT tool_name FROM tool_metrics"
    ).fetchall()
    assert [t[0] for t in tools] == ["explore"], "prior tool_metrics lost"


def test_copy_telemetry_tables_appends_with_fresh_ids(tmp_path):
    """copy_telemetry_tables appends rows with fresh ids, order preserved.

    The staged-build path already holds THIS build's build_runs row when the
    carry runs, so the helper must not copy ids (collision) and must preserve
    source order so time-ordered consumers stay correct.
    """
    from cairn.graph.schema import _apply_schema, copy_telemetry_tables

    src = str(tmp_path / "old.db")
    dst = str(tmp_path / "new.db")

    src_conn = sqlite3.connect(src)
    _apply_schema(src_conn)
    src_conn.executemany(
        "INSERT INTO build_runs (kind, started_at, duration_s) VALUES (?, ?, ?)",
        [
            ("sync", "2026-08-01T00:00:00Z", 1.0),
            ("embed", "2026-08-02T00:00:00Z", 2.0),
        ],
    )
    src_conn.commit()
    src_conn.close()
    ...
    _apply_schema(dst_conn)
    dst_conn.execute(
        "INSERT INTO build_runs (kind, started_at, duration_s) "
        "VALUES ('build', '2026-08-03T00:00:00Z', 3.0)"
    )
    dst_conn.commit()
    copy_telemetry_tables(dst_conn, src)
    rows = dst_conn.execute(
        "SELECT id, kind, started_at FROM build_runs ORDER BY started_at"
    ).fetchall()
    dst_conn.close()

    assert [r[1] for r in rows] == ["sync", "embed", "build"]
    # Ids are a permutation of 1..3 (fresh assignment, no collisions).
    assert sorted(r[0] for r in rows) == [1, 2, 3]


def test_copy_telemetry_tables_tolerates_pre_telemetry_db(tmp_path):
    """A source DB without the analytics tables carries nothing, quietly.

    Old (pre-T08) databases lack build_runs/events; the carry is best-effort
    and must not fail the swap.
    """
    from cairn.graph.schema import copy_telemetry_tables

    src = str(tmp_path / "pre_t08.db")
    dst = str(tmp_path / "fresh.db")

    src_conn = sqlite3.connect(src)
    src_conn.execute("CREATE TABLE symbols (id TEXT PRIMARY KEY)")
    src_conn.commit()
    src_conn.close()
    ...
    dst_conn.commit()
    copy_telemetry_tables(dst_conn, src)
    n = dst_conn.execute("SELECT COUNT(*) FROM build_runs").fetchone()[0]
    dst_conn.close()
    assert n == 0

    # A missing source file entirely is also fine (first-ever build).
    dst_conn = sqlite3.connect(dst)
    copy_telemetry_tables(dst_conn, str(tmp_path / "never_existed.db"))
    dst_conn.close()

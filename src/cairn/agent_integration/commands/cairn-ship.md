...
1. Run `cg update` to refresh the graph with any code changes (mutation — CLI only)
...
4. Run `cg memory batch-critic` to process pending drafts (bulk mutation — CLI only)
5. If any memory scored above 0.7, suggest:
   `cg memory promote <path>` (bulk tier change — CLI only)

...
# store for this workspace from cwd (ancestor walk); when the store home is not
# the default, the export line above pins it for this script.
...
def _render_post_commit(repo: str) -> str:
    ...
    if not env:
        return rendered
    path = env["CAIRN_HOME"]
    shebang, _, rest = rendered.partition("\n")
...
def install_hooks(repos: List[str], workspace: str, cairn_dir: str = "") -> List[str]:
    ...
    for repo in repos:
        ...
        if hook_path.exists() and "cairn" not in hook_path.read_text():
            continue
        hook_path.write_text(_render_post_commit(repo), encoding="utf-8")

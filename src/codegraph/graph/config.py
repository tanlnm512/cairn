"""User configuration for codegraph indexing.

Reads a ``codegraph.json`` at the workspace (or repo) root. Supports
...
def load_config(root: Union[str, Path]) -> CodeGraphConfig:
    """Load ``codegraph.json`` from ``root`` (workspace or repo dir).

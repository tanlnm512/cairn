...
`reindex_paths` is the common entry point for both `cg update` (git-diff) and
...
def reindex_paths(
    ...
    for abs_path in paths:
        ...
        # infer_repo_for_path here returns the directory name ('codegraph').
...
def incremental_update(
    ...
    concurrently-running `cg serve` processes (SSE daemon + per-editor stdio
...
def _changed_source_files(repo_path: Path, conn=None) -> List[str]:
    ...
    such repos silently report "0 changed files" on every ``cg update`` — the
    ...
    # comparison against the files table — the same signal `cg sync` uses.
    if conn is None:
        return []
...
def _changed_via_stat(repo_path: Path, conn) -> List[str]:
    ...
    Mirrors the logic in ``cg sync`` (cli/system.py): a file is "changed" if
...
def _reindex_file(
    ...
    Single-file path kept for backward compat (used by `cg update --file`).

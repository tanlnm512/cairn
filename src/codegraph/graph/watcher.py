"""Boot-time freshness: catches up the graph with disk edits at `cg serve` start.

**A long-running `cg serve` process does NOT see source edits made after it
...
(or run `cg build`/`cg update` from the CLI, which re-indexes directly
rather than going through this module) to pick up mid-session changes.

Architecture:
    1. `ensure_fresh_force(conn, workspace)` — called once, at `cg serve`
...
def ensure_fresh_force(conn, workspace: str) -> int:
    ...
    actually runs today -- called once, at `cg serve` boot. See module

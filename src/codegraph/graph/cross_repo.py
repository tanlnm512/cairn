...
``CODEGRAPH_REPO_NAMESPACES`` env var (JSON), then the ``repo_namespaces`` key
of the workspace's ``codegraph.json``, then the built-in :data:`_DEFAULT_NAMESPACES`
...
# the env var nor codegraph.json supplies a namespace map. Kept so existing
...
def _load_namespaces() -> Dict[str, str]:
    ...
    Priority (first non-empty wins):
      ...
      2. ``repo_namespaces`` in the workspace's ``codegraph.json``.
    ...
    # 1. Env override.
    env_raw = os.environ.get("CODEGRAPH_REPO_NAMESPACES")
    if env_raw:
        try:
            parsed = json.loads(env_raw)
        except json.JSONDecodeError as e:
            print(f"warning: CODEGRAPH_REPO_NAMESPACES: invalid JSON ({e}); "
        ...
        if isinstance(parsed, dict):
            ...
            print("warning: CODEGRAPH_REPO_NAMESPACES must be a JSON object; "
                  "ignoring", file=sys.stderr)

    # 2. codegraph.json at the resolved workspace root.
    try:

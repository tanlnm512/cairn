...
Holds the single ``FastMCP("codegraph")`` instance every tools_*.py module
...
from codegraph.graph.queries import find_definition
...
async def app_lifespan(server: FastMCP):
    ...
    Reads CODEGRAPH_DB / CODEGRAPH_KNOWLEDGE (set by ``cg serve``) or falls
    ...
    store = resolve_store()
    ctx = AppContext(
        db_path=os.environ.get("CODEGRAPH_DB") or str(store.db),
        knowledge_path=os.environ.get("CODEGRAPH_KNOWLEDGE") or str(store.knowledge),
...
mcp = FastMCP("codegraph", lifespan=app_lifespan)


def _store():
    """Resolve the central store for this workspace.

    Honors CODEGRAPH_DB / CODEGRAPH_KNOWLEDGE if set (cg serve sets both);
...
def _read_only_mode() -> bool:
    ...
    Set by `cg serve run --read-only` (and the launchd daemon plist) via the
    CODEGRAPH_READ_ONLY env var. When true, _conn() opens the DB read-only so
    ...
    """
    return os.environ.get("CODEGRAPH_READ_ONLY", "").lower() in ("1", "true", "yes")


def _conn():
    """Open a SQLite connection to the graph DB for this workspace.

    Read-only when CODEGRAPH_READ_ONLY is set (the safe shared-daemon mode);
    ...
    """
    return get_db(
        os.environ.get("CODEGRAPH_DB") or str(_store().db),
...
def _rw_conn():
    ...
    """
    return get_db(os.environ.get("CODEGRAPH_DB") or str(_store().db), read_only=False)


def _bundle() -> OKFBundle:
    """Build an OKFBundle for the current workspace's .knowledge/ dir."""
    knowledge = os.environ.get("CODEGRAPH_KNOWLEDGE") or str(_store().knowledge)
...
def _staleness_banner(conn, file_paths) -> str:
    ...
    long-running ``cg serve`` doesn't silently answer from a stale graph.
    ...
    return (
        ...
        f"incomplete until `cg update` runs. "  # trailing space separates from following output
    )
...
def status_resource() -> str:
    ...
    and the DB path. A client reads this via ``read_resource("codegraph://status")``
    to decide whether to trust a graph query or first prompt ``cg update``.
    """
    import json
    ...
    try:
        conn = _conn()
        try:
            from codegraph.graph.stats import get_stats

            stats = get_stats(conn)
        finally:
            conn.close()
    except Exception as e:
        return f"codegraph status: unavailable ({e})"
    ...
    db_path = os.environ.get("CODEGRAPH_DB") or str(_store().db)
    ...
    lines = [
        ...
        + ("  ⚠ stale -- run `cg update`" if stale_count else "  ✓ fresh"),
    ]

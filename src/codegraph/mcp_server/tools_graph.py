...
def impact_analysis(
    ...
    `cg build`/`cg sync`). Falls back to live analysis if no cache entry.
    ...
    try:
        ...
        if cached:
            from codegraph.graph.dataflow import get_dataflow as _get_dataflow
            ...
            if df is not None:
                ...
                out.append("(from precomputed cache — run `cg dataflow build` to refresh)")
                return "\n".join(out)
...
def impact_analysis_data(result: dict, xref: dict, *, name: str, fuzzy: bool, limit: int) -> dict:
    ...
    from codegraph.graph.tests import filter_tests as _filter_tests
...
def semantic_search(query: str, limit: int = 20, include_callers: bool = False, structured: bool = False) -> str | SemanticSearchResult:
    ...
    extra (pip install 'cg-intel[semantic]'), or set CODEGRAPH_EMBED_BACKEND=hash
    for a dep-free smoke test.

    By default (CODEGRAPH_FUSION unset or not "0"), results blend BM25 +
    ...
    match strength, set CODEGRAPH_FUSION=0 to get true 0..1 cosine scores.

    Set CODEGRAPH_RERANK=1 to add a cross-encoder rerank stage on top of the
    ...
    try:
        ...
        # (`cg embed`), not a query-time one. If the corpus isn't indexed yet,
        # tell the caller to run `cg embed` once.
        if emb.embed_count(conn) == 0:
            return (
                "Semantic index is empty. Run `cg embed` once to index the "
...
def _render_semantic_search(data: dict, include_callers: bool = False) -> str:
    ...
    rows = data["matches"]
    if not rows:
        fusion_on = os.environ.get("CODEGRAPH_FUSION", "1") != "0"
        return (
            ...
            "The corpus may not be embedded yet (run `cg embed` to index), or the "
...
def visualize_graph(
    ...
    """
    from codegraph.viz import query as vq
    from codegraph.viz import renderers as vr

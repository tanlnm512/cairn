"""Lifecycle management for the codegraph SSE daemon (macOS launchd).

Provides the plumbing for `cg serve start|stop|status|restart` so a single
...
Design:
- A LaunchAgent plist at ~/Library/LaunchAgents/dev.codegraph.sse.plist runs
  `cg serve --port <N>` with KeepAlive (restart on crash) and RunAtLoad
...
- `stop` does `launchctl unload` (graceful SIGTERM from launchd) and also
  sweeps stray `cg serve` processes that aren't under launchd -- the
...
RuntimeError with a clear message; callers can fall back to `cg serve`
...
def log_path() -> Path:
    return Path.home() / "Library" / "Logs" / "codegraph-sse.log"
...
def cg_bin() -> str:
    ...
    for cand in (
        os.environ.get("CODEGRAPH_BIN"),
        shutil_which("cg"),
        str(Path.home() / ".local" / "bin" / "cg"),
    ):
        if cand and Path(cand).exists():
            return cand
    return "cg"  # let launchd's PATH resolve it
...
def render_plist(
    ...
    Runs `cg serve run --port <N> --read-only` (the foreground SSE subcommand).
    Note the `run` subcommand: the top-level `cg serve` group doesn't accept
    ...
    never contends with `cg build`/`cg embed`/`cg memory`. Serving-time write
    ...
    bin_ = cg_bin()
    env = {
        # Inherit the user PATH so `cg` can find python etc. launchd
        # otherwise starts with a minimal environment.
        "PATH": os.environ.get("PATH", "/usr/local/bin:/usr/bin:/bin"),
    }
    if workspace:
        env["CODEGRAPH_WORKSPACE"] = str(workspace)
    if db_path:
        env["CODEGRAPH_DB"] = str(db_path)
    if knowledge_path:
        env["CODEGRAPH_KNOWLEDGE"] = str(knowledge_path)
...
def find_strays(db_path: str | Path) -> list[int]:
    """Find `cg serve` PIDs NOT managed by launchd that hold the DB.
    ...
    The match is scoped two ways for safety:
      - by command shape: only real ``cg serve run``/``cg serve``
        invocations (a bare ``cg serve`` substring would otherwise match ANY
        process whose argv merely contains it -- editors, grep, this very
        process);
      - by db_path: only processes serving the SAME DB. A server sets
        ``CODEGRAPH_DB`` (see cli/serve.py) and may also pass ``--db``; we
        ...
        appear literally in argv), we fall back to matching ``cg serve run``.
    """
    pids: list[int] = []
    ...
    # (so unrelated `cg serve` processes serving OTHER dbs are spared); we
...
def sweep_strays(db_path: str | Path, log: bool = False) -> int:
    """Find and kill stray `cg serve` processes. Returns count killed.

    Idempotent and safe to call from the daemon's background sweeper thread or
    from `cg serve start`/`stop`. Best-effort: a pid that died between find and
    kill is a no-op.
    """
    ...
    for pid in strays:
        ...
        if log:
            ...
            print(f"[{ts}] codegraph: stray sweeper killed orphan cg serve pid {pid}",
...
def load() -> bool:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"{path} not found. Run `cg serve start` first to create it."
        )
    r = subprocess.run(
...
def sse_responds(port: int = DEFAULT_PORT, host: str = DEFAULT_HOST, timeout: float = 2.0) -> bool:
    ...
    lock during a write) and can't service the request. That left `cg serve

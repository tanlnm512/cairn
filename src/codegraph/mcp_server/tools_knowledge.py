...
def knowledge_search(query: str, limit: int = 20) -> str:
    ...
    if not results:
        return (
            ...
            f"corpus isn't embedded yet (run `cg knowledge embed`). Try broader "
...
def knowledge_delete(doc_id: str) -> str:
    ...
    from codegraph.knowledge.store import get_document
...
def trace_workflow(ref: str) -> str:
    ...
    from codegraph.knowledge.workflow import trace_workflow as _trace
    ...
    if result is None:
        return (
            ...
            "or list workflows via `cg knowledge list --type workflow`."
        )

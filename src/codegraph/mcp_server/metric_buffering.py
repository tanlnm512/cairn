...
takes a SQLite write lock that contends with ``cg memory search`` /
``recall_memory`` (which also write) and with other ``cg serve`` processes
...
MAX_RESULT_CHARS = int(os.environ.get("CODEGRAPH_MAX_RESULT_CHARS", "60000"))
...
def _log_metric(tool_name: str, duration_ms: float, status: str = "ok",
    ...
    row = (
        tool_name,
        os.environ.get("CODEGRAPH_SESSION", "unknown"),
        time.time(),
        duration_ms,
        status,
...
def instrument(fn):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            parts_to_remove = [
                "/Projects/", "/codegraph/", "/src/", "/.knowledge/",

...
``codegraph.parsers.v1`` entry-point group. An entry point in that group must
...
change ships under ``codegraph.parsers.v2`` without breaking v1 plugins
...
def _load_language_capsule(language: str):
    ...
    try:
        ...
        # ``codegraph.parsers.v1`` entry-point group. Built-ins stay preferred
...
def _load_plugin_capsule(language: str):
    """Look up ``language`` in the ``codegraph.parsers.v1`` entry-point group.
...
_PLUGIN_ENTRY_POINT_GROUP = "codegraph.parsers.v1"

...
# Auto-generated by codegraph (cg hooks install)
# Runs an incremental graph update after every commit. cg resolves the central
# store for this workspace from cwd (ancestor walk), so no paths are baked in.
cg update --repo "{repo}" > /dev/null 2>&1 &
cg validate-paths --mark > /dev/null 2>&1 &
"""


def install_hooks(repos: List[str], workspace: str, codegraph_dir: str = "") -> List[str]:
    """Install post-commit hooks in each repo. Returns list of installed repo names.

    `codegraph_dir` is accepted but unused: the hook invokes `cg` (resolved on
    ...
    for repo in repos:
        ...
        # Don't overwrite a non-codegraph hook.
        if hook_path.exists() and "codegraph" not in hook_path.read_text():
            continue
...
def uninstall_hooks(repos: List[str], workspace: str) -> List[str]:
    """Remove codegraph post-commit hooks. Returns list of uninstalled repo names."""
    removed = []
    for repo in repos:
        hook_path = resolve_repo_path(workspace, repo) / ".git" / "hooks" / "post-commit"
        if hook_path.exists() and "codegraph" in hook_path.read_text():
            hook_path.unlink()
            removed.append(repo)
    return removed

...
and dispatches to the appropriate codegraph command.

Path-free: resolves the `cg` binary via PATH (shutil.which), falling back to
`python -m src.cli` using the running interpreter. Works regardless of install
...
def _cg_command() -> list[str]:
    """Resolve a cg invocation. Prefers a `cg` binary on PATH; falls back to
    `python -m src.cli` so the hook works from an editable install / source

...
from codegraph.graph import BASE_STOP_WORDS, simple_tokenize
...
def _cosine_scan(rows, q_blob, q_dim, threshold):
    ...
    Thin adapter over the shared ``codegraph.retrieval.cosine_scan`` so the
...
def _graph_bridge(conn, repos):
    ...
    from codegraph.graph import cross_repo_deps
    out = {}
    for repo in repos:
        try:

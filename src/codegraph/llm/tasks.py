...
that can read markdown and run `cg task` can process them — codegraph never
...
This decouples codegraph from any specific LLM/agent. The agent loads the
codegraph skill, reads a pending task, writes a result file, runs
`cg task complete`. Prompts live in the skill, keyed by task_kind.
"""
...
def _render_body(task: Task) -> str:
    ...
    lines.append(f"\nWrite your result, then run: `cg task complete {task.id}`")
    return "\n".join(lines) + "\n"
...
def _output_spec(task_kind: str) -> str:
    ...
    return specs.get(task_kind, "Process per the codegraph skill.")

...
# Slash commands provided by codegraph. Defined once and referenced everywhere
...
_MARKER = "codegraph"  # used to identify our entries when merging/uninstalling
...
def resolve_cg_command() -> list[str]:
    ...
    Prefers the absolute path of a `cg` binary on PATH (so the config works
    regardless of the client's cwd). Falls back to `python -m src.cli`.
    """
    cg = shutil.which("cg")
    if cg:
        return [cg]
...
def mcp_config_json(transport: str = "stdio", sse_url: str | None = None) -> dict:
    """MCP server config pointing at `cg serve` (shared mcpServers shape).
    ...
    When `cg` is on PATH (normal install), command is the absolute cg path and
    args is ["serve"]. When only the module fallback is available (dev/venv),
    command is the python interpreter and args is ["-m", "src.cli", "serve"].

    Args:
        transport: "stdio" (default, one process per client) or "sse" (one
            shared daemon, requires `cg serve start` to be running).
    ...
    if transport == "sse":
        ...
        return {"mcpServers": {"codegraph": {"type": "sse", "url": url}}}
    cmd = resolve_cg_command()
    if len(cmd) == 1:
        # cg binary: args = ["serve"]
        return {"mcpServers": {"codegraph": {"command": cmd[0], "args": ["serve"]}}}
    # module fallback (e.g. [python, "-m", "src.cli"]): append "serve" to args
    command, *prefix = cmd
    return {"mcpServers": {"codegraph": {"command": command, "args": [*prefix, "serve"]}}}
...
def _hook_markers() -> list[str]:
    """Substrings that identify a codegraph hook command, path-independently."""
    return [
...
def _claude_agent_md(template_name: str = "cursor/codegraph-explorer.json") -> str:
    ...
    `skills: ["codegraph"]` -- this preloads the full codegraph SKILL.md
    ...
    # mcp__codegraph__ prefix so Claude Code exposes them.
    mcp_tools = ", ".join(
        f"mcp__codegraph__{t}" for t in sub.get("tools", [])
    )
    if sub.get("readonly"):
    ...
    lines.append('mcpServers: ["codegraph"]')
    lines.append('skills: ["codegraph"]')
...
| `recall_memory` | Substring search across title + description + body + tags. Finds the query only if the exact words appear somewhere in the memory. A query for "messaging" won't find a memory about EventBus unless "messaging" appears in its text. | For concept-level queries, try broader search terms or put synonyms in memory titles/bodies. |
...
| `ann_backend_enabled` | Requires `CODEGRAPH_ANN_BACKEND=sqlite-vec` set explicitly -- not enabled just because `sqlite_vec` is importable. | Fully opt-in; leave the env var unset to stay on the brute-force cosine scan. |
...
def _claude_instructions() -> str:
    return (
        "# Codebase Intelligence (Codegraph)\n\n"
        "This workspace is connected to the codegraph knowledge system. Use these tools\n"

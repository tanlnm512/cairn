...
def opencode_mcp_config_json(transport: str = "stdio", sse_url: str | None = None) -> dict:
    ...
    3. For local servers, the full invocation is a single ``"command"`` ARRAY
       (e.g. ``["cg", "serve"]``) -- not separate ``command``/``args`` fields.
    ...
    if transport == "sse":
        ...
        return {"mcp": {"codegraph": {"type": "remote", "url": url, "enabled": True}}}
    cmd = resolve_cg_command() + ["serve"]
    return {"mcp": {"codegraph": {"type": "local", "command": cmd, "enabled": True}}}
...
def uninstall(ws: Path, res: InstallResult) -> None:
    """Remove codegraph entries from opencode.json and a stray ``.opencode/mcp.json`` if present."""

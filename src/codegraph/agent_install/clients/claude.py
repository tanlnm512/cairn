...
def install_claude(workspace: str, force: bool, dry_run: bool,
    ...
    """Wire codegraph into Claude Code (.mcp.json, .claude/, CLAUDE.md).

    ``scope="workspace"`` writes to ``<workspace>/.claude/`` and
    ``<workspace>/.mcp.json`` (default). ``scope="global"`` writes to
    ``~/.claude/`` and ``~/.mcp.json`` so all projects inherit codegraph
    ...
    _write_tree(base / ".claude" / "skills" / "codegraph", _TEMPLATE_DIR / "skill", force, res, dry_run=dry_run)
    ...
    _write_file(base / ".claude" / "agents" / "codegraph-explorer.md",
...
def uninstall(ws: Path, res: InstallResult) -> None:
    """Remove codegraph files/entries for Claude Code."""
    from ..merge import _strip_mcp

    _strip_mcp(ws / ".mcp.json", res)
    _rm_tree_if_codegraph(ws / ".claude" / "skills" / "codegraph", res)
    for n in _SLASH_COMMANDS:
        _rm_if_exists(ws / ".claude" / "commands" / f"{n}.md", res)
    _rm_if_exists(ws / ".claude" / "agents" / "codegraph-explorer.md", res)
    _rm_if_exists(ws / ".claude" / "agents" / "knowledge-steward.md", res)
    _rm_if_exists(ws / ".claude" / "agents" / "codegraph-agent.md", res)  # renamed; clean up the old filename

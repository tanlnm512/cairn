...
``{"mcp": {"servers": {"codegraph": {...}}}}`` shape (an explicit ``"type"``
...
def zcode_mcp_config_json(transport: str = "stdio", sse_url: str | None = None) -> dict:
    ...
    if transport == "sse":
        ...
        return {"mcp": {"servers": {"codegraph": {"type": "sse", "url": url}}}}
    cmd = resolve_cg_command()
    if len(cmd) == 1:
        return {"mcp": {"servers": {"codegraph": {"type": "stdio", "command": cmd[0], "args": ["serve"]}}}}
    command, *prefix = cmd
    return {"mcp": {"servers": {"codegraph": {"type": "stdio", "command": command, "args": [*prefix, "serve"]}}}}
...
def install_zcode(workspace: str, force: bool, dry_run: bool,
    ...
    """Wire codegraph into ZCode (.zcode/config.json, .zcode/ skill + commands).

    ZCode reads <base>/.zcode/config.json and expects the nested shape
    ``{"mcp": {"servers": {"codegraph": {...}}}}`` — NOT the top-level
    ...
    _write_tree(base / ".zcode" / "skills" / "codegraph", _TEMPLATE_DIR / "skill", force, res, dry_run=dry_run)
    ...
    _write_file(base / ".zcode" / "agents" / "codegraph-explorer.md",
    ...
    res.notes.append("ZCode hook schema differs; use `cg hooks install` for git automation.")
    return res


def uninstall(ws: Path, res: InstallResult) -> None:
    """Remove codegraph files/entries for ZCode."""
    _strip_mcp_zcode(ws / ".zcode" / "config.json", res)
    _rm_tree_if_codegraph(ws / ".zcode" / "skills" / "codegraph", res)
    for n in _SLASH_COMMANDS:
        _rm_if_exists(ws / ".zcode" / "commands" / f"{n}.md", res)
    _rm_if_exists(ws / ".zcode" / "agents" / "codegraph-explorer.md", res)

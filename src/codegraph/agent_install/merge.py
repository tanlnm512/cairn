...
and strip codegraph entries from JSON config files for ANY client. Each
...
def _merge_json_file(path: Path, merger: dict, force: bool, result: InstallResult, *,
    ...
    For .mcp.json: overwrites only the "codegraph" entry (config_key="mcpServers").
    For .zcode/config.json: overwrites the codegraph server under mcp.servers
    (config_key="zcode").
    For .claude/settings.json: merges hooks.<Event> lists, appending codegraph
    hook entries (skipping duplicates that already mention codegraph).
...
def _already_installed(existing: dict, merger: dict, *, config_key: str = "mcpServers") -> bool:
    """Has the codegraph entry already been written into `existing` *and matches*?

    For MCP (config_key="mcpServers"): checks mcpServers.codegraph.
    For ZCode (config_key="zcode"): checks mcp.servers.codegraph.
    For hooks: checks hooks.<Event> for codegraph hook commands.
    """
    if config_key == "zcode" and "mcp" in merger:
        cur = existing.get("mcp", {}).get("servers", {}).get("codegraph")
        if not cur:
            return False
        new = merger["mcp"]["servers"]["codegraph"]
    ...
    if config_key == "opencode" and "mcp" in merger:
        ...
        cur = existing.get("mcp", {}).get("codegraph")
        if not cur:
            return False
        new = merger["mcp"]["codegraph"]
    ...
    if "mcpServers" in merger:
        cur = existing.get("mcpServers", {}).get("codegraph")
        if not cur:
            return False
        new = merger["mcpServers"]["codegraph"]
        # Compare the full command (command + args joined) so path changes register.
        cur_cmd = cur.get("command", "") + " " + " ".join(cur.get("args", []))
        new_cmd = new.get("command", "") + " " + " ".join(new.get("args", []))
        if cur_cmd.strip() != new_cmd.strip():
            return False
        # Also compare env (Claude Desktop pins CODEGRAPH_WORKSPACE here). Absent
...
def _entry_present(entries: list, candidate: dict) -> bool:
    ...
    Matches on the codegraph hook entrypoint name (post_edit / session_end)
...
def _rm_tree_if_codegraph(path: Path, res: InstallResult) -> None:
    """Remove a dir tree only if it contains codegraph content."""
    if path.is_dir() and path.exists():
        shutil.rmtree(path)
        res.written.append(f"removed {path}/")


def _rm_if_codegraph(path: Path, res: InstallResult) -> None:
    if path.exists():
        path.unlink()
        res.written.append(f"removed {path}")
...
def _strip_mcp(path: Path, res) -> None:
    """Remove the codegraph server from an mcp.json, leaving others intact."""
    if not path.exists():
        return
    try:
    ...
    if "codegraph" in servers:
        del servers["codegraph"]
        data["mcpServers"] = servers
        path.write_text(json.dumps(data, indent=2) + "\n", encoding="utf-8")
        res.written.append(f"stripped codegraph from {path}")


def _strip_mcp_zcode(path: Path, res) -> None:
    """Remove the codegraph server from a .zcode/config.json, leaving others intact.

    Cleans up empty ``mcp`` and ``servers`` keys when codegraph was the only
...
def _strip_mcp_opencode(path: Path, res) -> None:
    """Remove the codegraph server from an opencode.json, leaving others intact.

    opencode's MCP shape is a flat ``mcp.<name>`` dict (no nested ``servers``
    key like ZCode). Cleans up the empty ``mcp`` key when codegraph was the
    ...
    if stray.exists():
        ...
        try:
            ...
            if "codegraph" in (stray_data.get("mcpServers") or {}):
                stray.unlink()
...
def _strip_hooks(path: Path, res: InstallResult) -> None:
    """Remove codegraph hook entries from .claude/settings.json.
    ...
    for event in list(hooks):
        ...
        for entry in entries:
            ...
            if any(isinstance(h, dict)
                   ...
                   for h in inner):
                changed = True
                continue  # drop this codegraph entry
            kept.append(entry)
        if kept:
            hooks[event] = kept
        else:
    ...
    if changed:
        if hooks:
            ...
            data.pop("hooks", None)
        path.write_text(json.dumps(data, indent=2) + "\n", encoding="utf-8")
        res.written.append(f"stripped codegraph hooks from {path}")


def _strip_cursor_hooks(path: Path, res: InstallResult) -> None:
    """Remove codegraph entries from .cursor/hooks.json."""
    if not path.exists():
        return
    try:
        data = json.loads(path.read_text(encoding="utf-8"))

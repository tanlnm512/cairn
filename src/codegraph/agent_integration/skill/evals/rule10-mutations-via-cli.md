...
wrong_calls:
  - tool: mcp__codegraph__rebuild_graph
    kind: mcp
...
MCP tools are **read-only for L1** (the codegraph itself: queries,
...
An agent attempts `mcp__codegraph__rebuild_graph` (or similar), gets a

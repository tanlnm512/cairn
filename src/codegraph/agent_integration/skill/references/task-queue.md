# Codegraph: Task Queue — Processing LLM Synthesis Tasks

Codegraph is **agent-decoupled**: it never calls an LLM directly. Instead it
...
5. **Submit it** — codegraph stores the result and the critic runs:
   ```bash
   cg task complete <task_id> --result-file <path>
   ```

6. If the critic found factual errors, codegraph queues a `*-revise` task

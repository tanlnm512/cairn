#!/usr/bin/env python3
"""Run `cg impact` and guard against name-collision blowups (Golden Rule 6).

`impact_analysis`/`cg impact` traverses callers recursively by name. Common
...
to call from an agent without extra error handling; a non-zero `cg impact`
...
def _cg_command() -> list[str]:
    ...
    Mirrors agent_install.resolve_cg_command(): prefer the `cg` binary on
    PATH; fall back to `python -m src.cli` when it isn't (e.g. an editable
...
def run_cg(args: list[str]) -> subprocess.CompletedProcess:
    try:
        ...
        print("error: `cg` not found on PATH", file=sys.stderr)
        sys.exit(1)
...
def try_dataflow_fallback(symbol: str) -> None:
    ...
    if df.returncode != 0:
        print("  (no precomputed dataflow entry -- run `cg dataflow build` first, "

...
def embed(db, batch_size, limit, no_reap, build_index, install_deps, download_model):
    ...
    if install_deps:
        ...
        # MB). Run `cg embed` separately to build the index.
        ...
        display.dim("Run `cg embed` to build the embedding index.")
        sys.exit(0)

    if not emb.embeddings_available():
        display.error("Semantic dependencies unavailable")
        display.dim(emb.install_hint())
        display.dim("Run `cg embed --install-deps` to auto-install.")
        sys.exit(1)

    # Warn when silently falling back to the hash backend. The fallback is
    # intentional for `semantic_search` (graceful degradation), but `cg embed`
    # is an explicit action where the user expects real model embeddings.
    # If CODEGRAPH_EMBED_BACKEND is unset (default 'local') but sentence-
    ...
    if emb._effective_backend() == "hash" and not os.environ.get("CODEGRAPH_EMBED_BACKEND"):
        display.warning(
        ...
        display.dim(
            "This downloads torch + sentence-transformers into ~/.codegraph/lib "
            "(survives reinstalls). Then re-run: cg embed"
        )
        display.dim(
            "Or set CODEGRAPH_EMBED_BACKEND=hash explicitly to silence this warning."
        )
...
def semantic(query, db, limit, threshold, as_json, include_callers):
    ...
    Set CODEGRAPH_RERANK=1 to add a cross-encoder rerank stage (widens the
    ...
    try:
        ...
        # build-time op (`cg embed`); point the user there if not indexed.
        if emb.embed_count(conn) == 0:
            from . import display
            display.warning(
                f"Semantic index is empty under model '{emb.current_model()}'. "
                "Run `cg embed` once to index, then retry."
            )
            sys.exit(1)

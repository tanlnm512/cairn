...
@click.option("--force", is_flag=True, help="Overwrite existing codegraph files.")
...
@click.option("--sse", "sse", is_flag=True,
              help="Force SSE configs (shared daemon). Run `cg serve start` first.")
...
@click.option("--yes", "-y", is_flag=True,
              help="Skip the interactive prompt; install for detected clients that don't already have codegraph.")
def install_agents(clients, ws_arg, scope_arg, force, dry_run, git_hooks, sse, stdio, sse_url, yes):
    """Wire codegraph into AI coding clients (Claude Code/Desktop, Cursor, Droid, ZCode, etc.).

    Detects which clients are installed and shows whether codegraph is already
    wired in. By default, prompts you to choose which clients to install for
    (skipping those that already have codegraph). Use --client to bypass the
    ...
    for d in detections:
        ...
        click.echo(f"  [{mark}] {d.client:16} {d.reason:30}  codegraph: [{inst_mark}] {inst}")
    click.echo("")
    ...
    if cl:
        ...
        if not not_yet_installed:
            click.echo("All detected clients already have codegraph installed.")
        ...
        if yes or not sys.stdin.isatty():
            ...
            click.echo(f"Install codegraph for which clients?")
    ...
    click.echo("")
    click.echo("Done. The MCP server resolves its own store paths via `cg serve`.")
...
def uninstall_agents(clients, ws_arg):
    """Remove codegraph entries from AI client configs. Idempotent.

    Strips the codegraph MCP server and hooks from config files (preserving
    other entries) and deletes codegraph skill/command/subagent files.
    """
    ...
    click.echo("")
    click.echo("Done. Git hooks: run `cg hooks uninstall` separately if needed.")

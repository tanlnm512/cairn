...
@click.option("--from-legacy", "legacy_dir", default=None,
              help="Migrate .kg + .knowledge from this codegraph dir (e.g. ./codegraph).")
...
def init(ws_arg, legacy_dir, no_build, import_docs):
    """Register this workspace with codegraph's central store.

    Creates ~/.codegraph/<key>/.kg and .knowledge/ for this workspace and
    records cwd -> key in the registry, so subsequent `cg` commands (and the
    MCP server) find the right store from anywhere inside the workspace.

    If a legacy codegraph/.kg exists under the workspace, it is migrated into
    ...
    # Migrate legacy codegraph/.kg + .knowledge if present.
    legacy = Path(legacy_dir).resolve() if legacy_dir else ws / "codegraph"
    legacy_db = legacy / ".kg"
    ...
    if not no_build and not store.db.exists():
        ...
        click.echo("Graph already present. Use `cg build` to rebuild.")
    ...
    click.echo("\nDone. `cg config` to verify; `cg serve` for the MCP server.")
...
def config(list_all, mcp_config):
    ...
    if mcp_config:
        snippet = {
            "mcpServers": {
                "codegraph": {"command": "cg", "args": ["serve"]}
            }
        }
    ...
    if list_all:
        ...
        reg = _load_registry()
        if not reg:
            click.echo(f"No workspaces registered. Run `cg init` in a workspace root.")
    ...
    click.echo(f"  .knowledge:  {store.knowledge}")
    click.echo(f"home:       {store.home.parent}  (override with CODEGRAPH_HOME)")
    ...
    # surfaces the state before the user ever runs `cg embed`.
    ...
    if eff == "hash" and configured == "local":
        ...
        click.echo(f"            for real embeddings: uv tool install 'cg-intel[semantic]' --force")
    elif eff == "hash":
        click.echo(f"embed:      {model}  (CODEGRAPH_EMBED_BACKEND=hash)")
    else:
        click.echo(f"embed:      {model}  (backend: {eff})")

    # Show the resolved cross-repo namespace map (env / codegraph.json / default).
    from ..graph.cross_repo import _load_namespaces
    from ..graph.config import load_config

    cfg = load_config(store.workspace)
    source = "CODEGRAPH_REPO_NAMESPACES" if os.environ.get("CODEGRAPH_REPO_NAMESPACES") else (
...
def build(repo, workspace, db, verbose, staging):
    ...
    try:
        ...
        # SQLite's writer lock and can lock out subsequent `cg build`/`cg embed`.
        if conn is not None:
            conn.close()
    ...
    if verbose and summary.get("skipped"):
        display.dim(f"{summary['skipped']} files skipped (gitignored/default/config/size; see `cg stats`)")
...
def checkpoint(db):
    ...
    The codegraph server runs in WAL mode. With multiple processes (or a
    ...
    Run this after `cg serve stop` to reclaim space, or as recovery if the

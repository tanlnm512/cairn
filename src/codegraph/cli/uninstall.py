"""Uninstall CLI: ``cg uninstall`` — full teardown, native (no shell script).
...
def _home() -> Path:
    """CODEGRAPH_HOME read lazily, so tests/processes that set the env var
    after import are honored. (paths.py binds it once at import time.)"""
    return Path(os.environ.get("CODEGRAPH_HOME", str(Path.home() / ".codegraph")))


# ─── detection helpers ─────────────────────────────────────────────────────

def _detect_install_method() -> str:
    """How cg-intel was installed: 'uv', 'pipx', 'pip', 'venv', or 'unknown'.
    ...
    try:
        ...
        pkg_root = Path(find_spec("codegraph").origin).resolve().parents[2]
...
def _resolve_store_target(ws: str, full: bool) -> tuple[Path, bool] | None:
    ...
    - workspace not pinned, but home has stores -> (home, True)
      ...
      ~/.codegraph clearly holds stores — the user is running from the tool
...
def _remove_store(ws: str, full: bool, dry_run: bool) -> None:
    ...
    if resolved is None:
        click.echo("  (no codegraph store found — nothing to remove)")
        return
    ...
    if whole_home:
        ...
        label = f"{target} (entire codegraph home, {n} workspace(s))"
    else:
        label = str(target)
...
def _remove_binary(installed_via: str, dry_run: bool) -> None:
    ...
    if dry_run:
        cmds = {
            "uv": ["uv", "tool", "uninstall", "cg-intel"],
            "pipx": ["pipx", "uninstall", "cg-intel"],
            "pip": [sys.executable, "-m", "pip", "uninstall", "-y", "cg-intel"],
            "venv": ["rm -rf <pkg>/.venv"],
        }
    ...
    if installed_via == "uv":
        subprocess.run(["uv", "tool", "uninstall", "cg-intel"], check=False)
    elif installed_via == "pipx":
        subprocess.run(["pipx", "uninstall", "cg-intel"], check=False)
    elif installed_via == "pip":
        subprocess.run([sys.executable, "-m", "pip", "uninstall", "-y", "cg-intel"], check=False)
...
@click.option("--full", "full", is_flag=True,
              help="Remove everything for ALL workspaces (the entire ~/.codegraph).")
...
def uninstall(full, agents_only, hooks_only, graph_only, package_only, clients, ws_arg, dry_run, yes):
    """Uninstall codegraph: agent wiring, hooks, graph store, and the cg binary.

    By default removes everything for the CURRENT workspace. Use --full to wipe
    the entire ~/.codegraph (all workspaces' stores). CLAUDE.md / AGENTS.md and
    ...
    if ws_arg:
        ws = ws_arg
    elif os.environ.get("CODEGRAPH_WORKSPACE"):
        ws = os.environ["CODEGRAPH_WORKSPACE"]
    else:
        ws = str(Path.cwd())

    # Decide which steps run. A bare `cg uninstall` runs all four; any --*-only
    ...
    # --full targets the entire CODEGRAPH_HOME regardless of which workspace
    # we're in. Otherwise the resolved workspace's own store (or, if it has
    # none, the whole home — see _resolve_store_target).

    click.echo("")
    click.secho("Codegraph Uninstaller", bold=True)
    if dry_run:
    ...
    click.echo("")
    click.echo("  To reinstall: pip install cg-intel  (or ./scripts/install.sh)")

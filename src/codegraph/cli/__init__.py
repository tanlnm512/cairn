"""cg CLI: codegraph commands.

All codegraph functionality is exposed via the `cg` command.
Run `cg --help` for the full command list, or `cg version` to check
...
# Re-export main for `from codegraph.cli import main` (entry point in pyproject.toml
# is `src.cli:main`, and tests import it directly).
